...
Every published figure is generated by code, in one shared visual language:
white background, Helvetica-first sans, no top/right spines, muted greys for
the frozen baselines, and `#1f5fd8` reserved for Online-SDFT. `METHOD_COLORS`
and `METHOD_DASHES` in `online_sdft/reporting.py` are the single source for
method styling, and the site accent matches them.

| Figure | Generated by |
| --- | --- |
| `figures/blog_teaser.png` | `python3 website/make_diagrams.py --only teaser` |
| `figures/evidence_gap.png` | `python3 website/make_diagrams.py --only evidence` |
| `figures/bandit_accuracy.png` | `python3 run.py --figures-only` |
| `figures/bandit_learning_curves.png` | `python3 run.py --figures-only` |
| `figures/online_sdft_process.gif` | `python3 build_standalone_notebook.py` |

`website/make_diagrams.py` with no arguments redraws both hand-laid diagrams.
Hand-laid diagrams are preferred over Mermaid: the builder still promotes
Mermaid blocks if `BLOG.md` contains any, but it only loads the Mermaid runtime
when a block is actually present.
...
The workflow runs the site builder and copies those figures into `_site`; add
any new figure to the `cp` list in `.github/workflows/pages.yml`.
